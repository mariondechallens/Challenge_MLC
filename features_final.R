  ...
  for (i in c(4:10))
  {
    ...
    df = as.data.frame(t(s))
     colnames(df) = c(paste0("wave1_ent_eeg",i-3),
                      paste0("wave1_sd_eeg",i-3),
                      paste0("wave2_ent_eeg",i-3),
                      paste0("wave2_sd_eeg",i-3),
                      paste0("wave3_ent_eeg",i-3),
                      paste0("wave3_sd_eeg",i-3),
                      paste0("wave4_ent_eeg",i-3),
                      paste0("wave4_sd_eeg",i-3))
                     
    
    if (train)
    {
      df =cbind(ytrain,df)
      #write.csv(df,file = paste0(data_folder,"wavelets_coeff_eeg_R4",i-3,".csv"),row.names = FALSE)
      write.csv(df,file = paste0("wavelets_coeff_eeg_R4",i-3,".csv"),row.names = FALSE)
     
      }
    else
      #write.csv(df,file = paste0(data_folder,"wavelets_coeff_eeg_R4",i-3,"_test.csv"),row.names = FALSE)
      write.csv(df,file = paste0("wavelets_coeff_eeg_R4",i-3,"_test.csv"),row.names = FALSE)
...
#rassembler les features dans un seul dataframe
rassembler_feat = function(train = TRUE)  # features sur ondelettes
{
  if (train)
  {
...
rassembler_feat_prop = function(train = TRUE)  #proportions ondes alpha etc
{
  if (train)
  {
...
rassembler_feat_freq = function(train = TRUE)  #features sur ondes alpha etc
{
  if (train)
  {

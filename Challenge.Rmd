...
#"wave2_sd_eeg2", écart-type sur les ondelettes 2 de l'EEG 2;
...
#"wave3_sd_eeg2", écart-type sur les ondelettes 3 de l'EEG 2; 
...
#"wave4_sd_eeg4", écart-type sur les ondelettes 4 de l'EEG 4; 
#"mean_abs_.eeg_7", moyenne de la valeur absolue du signal de l'EEG 7;  
#"wave3_sd_eeg4", écart-type sur les ondelettes 3 de l'EEG 4; 
...
#"wave4_sd_eeg2", écart-type sur les ondelettes 4 de l'EEG 2; 
#*"min_abs_.accelerometer_y", minimum de la valeur absolue du signal de l'accéléromètre suivant y; 
#"wave1_sd_eeg2", écart-type sur les ondelettes 1 de l'EEG 2.
```
...
La validation croisée est réalisée par l'algorithme de Random Forest lui-même (le "Out-of-the-bag error estimate").Pour chaque arbre simulé, deux tiers des données tiré aléatoirement sont utilisés pour l'apprentissage et le reste pour le test du modèle. A chaque simulation, l'algorithme simule l'arbre de classification, puis va classifier les données test à partir de cet arbre.
A la fin de l'algorithme, on regarde quelle observation "j"" a eu le meilleur score de proximité lorsque l'observation "n"" est "out-of-the-bag", ce qui nous permettra d'obtenir l'estimation d'erreur "out-of-the-bag" en normalisant avec le nombre d'arbre utilisé.
...
Pour le nombre de variables tirées aléatoirement avec remise à chaque étape de l'algorithme, nous avons choisi 48 car ce nombre est supérieur au nombre par défaut de l'algorithme, mais permettait d'obtenir un temps de calcul raisonnable.
...
```
Sur ce graphe nous avons présenté les variables de plus grande importance et leur score associé. Nous voyons donc que les cinq premières variables les plus importantes se démarquent beaucoup des autres en ayant un score largement supérieur aux autres, les autres ayant un score décroissant mais relativement proche les unes des autres.


```{r Plot OOB, echo = FALSE,fig.align='center'}
knitr::include_graphics('Rplot_out_ofbag.png')
```
Ce graphe présente la répartition de l'utilisation des données. On voit donc que la sélection aléatoire des données "out-of-the-bag" suit une gaussienne, de moyenne environ 255, ce qui correspond à un peu plus du tiers du nombre d'arbres. Ce résultat est cohérent puisqu'on cherche en théorie à tester chaque observations dans un tiers des cas. Notre simulation est donc cohérente.

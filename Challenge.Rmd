...
L'objet `xtrain` contient les onze datasets à exploiter. Pour ce faire, nous les transformons en `dataframe`.

```{r Données 2}
eeg1 = xtrain[list.datasets(xtrain, recursive = TRUE)[4]]
eeg1 = as.data.frame(readDataSet(eeg1))
eeg1_y = cbind(eeg1,ytrain[,2])
colnames(eeg1_y)[ncol(eeg1_y)] = "y"
```

On peut observer le premier enregistrement ci-dessous.
```{r Plot EEG1, echo = FALSE}
plot(1:ncol(eeg1), eeg1[1, ], type = 'l', ylab = "Valeur de l'EEG", xlab = "Numéro d'enregistrement sur 30 secondes", main = "EEG position 1 - enregist. 1")
```
